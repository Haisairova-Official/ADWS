...
class UpdateTests(unittest.TestCase):
    ...
    def test_cn_proxy_first_falls_back_to_github(self):
        with patch.object(update, 'mainland_china', return_value=True), patch.object(update, 'current_version', return_value='1.25 Release'), patch.dict(os.environ, {'ADWS_GITHUB_PROXY':'https://gh-proxy.com/'}), patch.object(update, 'read_json', side_effect=[ValueError('proxy error'), {'tag_name':'v1.26'}]) as read:
            ...
            self.assertEqual(result['url'], 'https://github.com/Haisairova-Official/ADWS/releases/tag/v1.26')
    ...
    def test_remote_release_links_cannot_redirect_to_another_site(self):
        with patch.object(update, 'mainland_china', return_value=False), patch.object(update, 'read_json', return_value={'tag_name':'v99.0','html_url':'https://example.invalid'}):
            self.assertTrue(update.check_update()['url'].startswith('https://github.com/Haisairova-Official/ADWS/releases/tag/'))
    ...
    def test_preview_proxy_failure_and_pagination(self):
        old={'tag_name':'v1.25'}
        with patch.object(update,'mainland_china',return_value=True), patch.dict(os.environ,{'ADWS_GITHUB_PROXY':'https://gh-proxy.com/'}), patch.object(update,'current_version',return_value='1.27-C'), patch.object(update,'read_json',side_effect=[OSError('proxy unavailable'), [old]*100, [{'tag_name':'v1.27-D','prerelease':True}]]) as read:
    ...
    def test_shell_dispatch_for_both_update_spellings(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            for flag in ['-u','--update']:
                result=subprocess.run(['bash',str(root/'adws'),flag,'--preview'],env={**os.environ,'PATH':folder+os.pathsep+os.environ['PATH']},capture_output=True,text=True,check=True)
                self.assertEqual(result.stdout.splitlines(),[str(root/'tools/adws_update.py'),'--preview'])
