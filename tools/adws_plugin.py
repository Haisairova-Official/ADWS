#!/usr/bin/env python3
"""adws-plugin — .mplg 插件包工具（drop-in 模式）

.mplg = zip，内含 plugin.json 与本体实现。把 .mplg 直接丢进插件目录即可被
ADWS 扫描加载，不需要“安装/注册”。规范见 docs/mplg-spec.md。
"""
...
import fcntl
import stat
import adws_plugin_api as api1
from pathlib import Path

MANIFEST = "plugin.json"
API_NAME = "adws-plugin"
API_VERSION = 1
...
def plugin_dir() -> Path:
    """扫描目录：用户把 .mplg 直接丢进来。"""
    override = os.environ.get("ADWS_PLUGIN_DIR")
    if override:
        return Path(override).expanduser()
    return Path(os.environ.get("XDG_DATA_HOME") or Path.home() / ".local/share") / "adws/plugins"


def cache_dir() -> Path:
    """运行缓存：zip 里的入口没法直接执行，宿主按需解包到这里。"""
    override = os.environ.get("ADWS_CACHE_DIR")
    if override:
        return Path(override).expanduser()
    return Path(os.environ.get("XDG_CACHE_HOME") or Path.home() / ".cache") / "adws/plugins"
...
def validate_manifest(manifest, members: set[str] | None = None) -> list[str]:
    ...
    if not errors:
        if not api1.ID.fullmatch(manifest["id"]):
            errors.append(_tr('id 必须为小写反向域名风格，如 org.adws.hello'))
...
def cmd_build(args) -> int:
    ...
    print(_tr('把它丢进 %s 即可被 ADWS 扫描加载。') % plugin_dir())
    return 0
...
def arguments(argv=None):
    ...
    p.add_argument("--id", help=_tr('插件 id（默认 org.adws.<目录名>）'))
