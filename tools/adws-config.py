#!/usr/bin/env python3
"""ADWS-Config — Akizuki's Desktop Workspace Solution 统一设置
"""
...
AUTOSTART_BEGIN = "// ==== ADWS 桌面图标层自启（自动生成）===="
AUTOSTART_END = "// ==== ADWS 桌面图标层自启 END ===="
...
def start_taskbar() -> tuple[bool, str]:
    from adws_runtime import start_taskbar as start
    return start(live_config_path(), live_style_path())


def restart_taskbar() -> tuple[bool, str]:
    from adws_layout import restart_taskbar as restart
...
def desktop_autostart_enabled() -> bool:
    ...
    try:
        ...
        return False
    from adws_autostart import desktop_enabled
...
def set_desktop_autostart(enabled: bool) -> tuple[bool, str]:
    ...
    from adws_autostart import set_desktop
    line = autostart_line()
    try:
    ...
    if enabled:
        return True, _tr('已写入 niri 自启：%s\n（下次登录生效；可立即运行 adws restart desktop 启动）') % line
...
class ConfigWindow(Gtk.Window):
    ...
    def build_about_page(self):
        box = dialog_box()
        transfer = Gtk.Box(spacing=12)
        for importing, caption in [(True, '导入全局配置'), (False, '导出全局配置')]:
        ...
        setup_button = Gtk.Button(label=_tr('重新运行设置向导'))
        def open_setup(*_):
            from adws_oobe import launch
            launch()
        setup_button.connect('clicked', open_setup)
        box.pack_start(setup_button, False, False, 0)
        title = Gtk.Label(label="Akizuki's Desktop Workspace Solution (ADWS)", xalign=0)
        title.get_style_context().add_class("title")
        box.pack_start(title, False, False, 0)
        ...
        font = Gtk.Label(label=_tr('字体') + "：%s / %dpx" % (prefs["font_family"], prefs["font_size"]), xalign=0)
        box.pack_start(font, False, False, 0)
        diagnostics = Gtk.Expander(label=_tr('诊断信息'))
        ...
        copy_button = Gtk.Button(label=_tr('复制诊断信息'))
        copy_button.set_halign(Gtk.Align.START)
        def copy_diagnostics(button):
            clipboard = Gtk.Clipboard.get(Gdk.SELECTION_CLIPBOARD)
            clipboard.set_text(text, -1)
            clipboard.store()
            button.set_label(_tr('已复制'))
        copy_button.connect('clicked', copy_diagnostics)
        details.pack_start(copy_button, False, False, 0)
        details.show_all()
        details.set_no_show_all(True)
        details.hide()
        diagnostics.connect('notify::expanded', lambda expander, _spec: details.set_visible(expander.get_expanded()))
    ...
    def check_updates(self, _button=None):
        import threading
        from adws_update import check_update
...
def keep_scroll_for_page(container, scroll):
    ...
    def attach(widget):
        if isinstance(widget, (Gtk.ComboBox, Gtk.SpinButton, Gtk.Scale)):
            if getattr(widget, "_adws_scroll_guard", False):
                return
            widget._adws_scroll_guard = True
...
class TaskbarSettingsWindow(Gtk.Window):
    def __init__(self, tab=None, layout_file=None, open_plugin=None):
        ...
        box.pack_start(row_widget(_tr('字体：'), self.family), False, False, 0)
        box.pack_start(row_widget(_tr('字号：'), self.font_size), False, False, 0)
        hint = Gtk.Label(label=_tr('仅作用于 ADWS 任务栏；竖栏以宽度作为厚度。其他组件始终保持单排。'), xalign=0)
        hint.get_style_context().add_class("dim-label")
        hint.set_line_wrap(True)
        box.pack_start(hint, False, False, 0)
        from adws_layout_gui import LayoutWindow
...
def main(argv=None):
    ...
    from adws_oobe import needed, main as setup_main
    if needed():
        old_argv = sys.argv
        try:
            sys.argv = [old_argv[0]]
            ...
            sys.argv = old_argv
        return 0
    Gdk.set_program_class("adws-config")
    Gtk.init([])
