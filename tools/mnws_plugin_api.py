...
from mnws_update import version_key, current_version
...
def normalize(manifest):
    ...
    if 'renderer' in result:
        result.update(api='mnws-plugin', apiVersion=1, kind='panel', language='python')
...
def public_errors(manifest):
    ...
    host = manifest.get('mnws', {})
    if not isinstance(host, dict): return errors + ['mnws must be an object']
