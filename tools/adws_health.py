...
def check(preinstall=False):
    ...
    if preinstall:
        ...
        try:
            from adws_layout import (load_layout, enabled_builtins, taskbar_library_path,
            ...
            for plugin in layout.get('plugins', []):
                if plugin.get('enabled'):
                    ...
                    if manifest is None:
                        ...
                        libraries.append(Path.home() / '.local/lib/waybar/libadws_panel.so')
    ...
    if not errors:
        try:
            ...
            if desktop.exists() and not desktop.is_dir():
                ...
                errors.append(''.join([_tr('缺少桌面目录：'), f'{desktop}', _tr('；运行 adws install 或启动桌面以初始化')]))
        except (ImportError, ValueError, OSError) as error:
