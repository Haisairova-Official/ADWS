...
DESKTOP_BEGIN = '// ==== ADWS 桌面图标层自启（自动生成）===='
DESKTOP_END = '// ==== ADWS 桌面图标层自启 END ===='
...
def compose(text, desktop, taskbar):
    for begin, end in [(BEGIN, END), (DESKTOP_BEGIN, DESKTOP_END)]:
        if text.count(begin) != text.count(end):
            raise ValueError(_tr('ADWS 自启标记不完整，请检查 niri 配置。'))
    ...
    blocks = []
    if taskbar:
        line = 'spawn-at-startup ' + json.dumps(str(ROOT / 'adws'), ensure_ascii=False) + ' "taskbar" "-s"'
        blocks.append('\n'.join([BEGIN, line, END]))
    if desktop:
...
def write_config(path, text):
    ...
    if path.read_text() == text:
        return
    fd, name = tempfile.mkstemp(prefix='.adws-autostart-', suffix='.kdl', dir=path.parent)
    temporary = Path(name)
    try:
...
def main():
    try:
        while True:
            answer = ask(_tr('是否让 ADWS 桌面和任务栏随 Niri 自启？（Y/n/Ctrl+C）')).lower()
            if answer in ('n', 'no'):
