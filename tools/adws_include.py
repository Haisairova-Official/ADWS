"""Normalize ADWS's single default include without rewriting user include lists."""
...
def detach_template_link(path):
    """Detach old installer links into ADWS templates, never arbitrary user links."""
    path = Path(path)
    ...
    if (target.name != path.name or target.parent.name != 'waybar'
            ...
            or not (target.parents[2] / 'tools/adws_layout.py').is_file()
            or not (target.parents[2] / 'adws').is_file()):
        return False
    shutil.copy2(target, path.with_name(path.name + '.adws-template-bak'))
    fd, temporary = tempfile.mkstemp(prefix='.adws-template-', dir=path.parent)
    os.close(fd)
    try:
...
def update_installed_config(path):
    ...
    backup = path.with_name(path.name + '.adws-include-bak')
    if not backup.exists():
        shutil.copy2(target, backup)
    fd, temporary = tempfile.mkstemp(prefix='.adws-include-', dir=target.parent)
    try:
