...
static GtkSizeRequestMode start_request_mode(GtkWidget *w) { (void)w; return GTK_SIZE_REQUEST_WIDTH_FOR_HEIGHT; }
...
static gboolean start_draw(GtkWidget *w, cairo_t *cr) {
    ...
    if (image) {
        double scale = (double)ih / gdk_pixbuf_get_height(image);
