...
#[derive(Debug)]
pub struct WindowSet(Option<Inner>, bool);

impl WindowSet {
    /// Creates a new window set.
    pub fn new(current_workspace_only: bool) -> Self {
        Self(None, current_workspace_only)
    }
...
impl Niri {
    ...
    /// Marks a workspace as active (and optionally focused), mirroring Niri's own event handling.
    ///
    /// Niri sends `WorkspaceActivated` when the active workspace changes on an output, rather
    /// than re-sending the full `WorkspacesChanged` list, so we have to patch the cached
    /// workspaces here or workspace switching won't be reflected in the snapshot.
    fn activate_workspace(&mut self, id: u64, focused: bool) {
        let Some(activated) = self.workspaces.get(&id) else {
            tracing::warn!(id, "got activated workspace not present in workspaces");
            return;
        };
    ...
    fn snapshot(&self, current_workspace_only: bool) -> Snapshot {
            ...
            .filter_map(|window| {
                if let Some(ws_id) = window.workspace_id {
                    if let Some(workspace) = self.workspaces.get(&ws_id) {
                        if !current_workspace_only || workspace.is_active {
