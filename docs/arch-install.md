# ADWS 1.30 Pre-Release — Arch Linux x86_64

解压 `ADWS1.30_Pre-Release_for_arch.zip`，进入解压目录并运行 `./install.sh`。
...
`adws -s` 启动桌面和任务栏，`adws --update` 检查正式 Release，
...
Use `adws -s` to start both components, `adws --update` to check stable Releases,
and `adws --uninstall` to remove ADWS. Waybar with CFFI v2 is still required.
