...
配置：`$XDG_CONFIG_HOME/mnws/taskbar-pins.json`（默认 `~/.config/mnws/taskbar-pins.json`）。保存应用 ID、桌面启动器 ID、名称和固定顺序，不保存易变的窗口 ID。写入加锁并原子替换；读取遇到无效文件时保留上一套有效配置。取消固定已卸载的应用不要求其启动器仍然存在。
